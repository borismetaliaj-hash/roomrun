...
// other way to come down short of touching Redis directly. Originally gated by the same
// owner-only login check as admin-stats.js (getUserFromRequest + OWNER_EMAIL).
//
// Self-service removal added 2026-08-04: the POST response now hands back the entry's id,
// which index.html remembers in localStorage on the submitter's own device. That id is never
// exposed via GET /api/listings (getCommunityListings() strips it out), so knowing it is proof
// you're either the original submitter or the app owner — no login needed for a student to
// remove their own listing. The owner-login path is kept as a fallback for clearing spam/junk
// by address if the id is ever lost.
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'DELETE') {
    try {
      ...
      const addressMatch = String(body.address || '').trim();
      if (!city || (!id && !addressMatch)) {
        res.status(400).json({ error: 'city and (id or address) are required' });
        return;
      }

      // Authorized either by knowing the listing's own id (handed back once, at creation, to
      // whoever submitted it — never exposed again via GET /api/listings) or by being logged
      // in as the app owner (fallback for cleaning up spam/junk by address).
      let authorized = !!id;
      if (!authorized) {
        const user = await getUserFromRequest(req).catch(() => null);
        authorized = !!user && user.email === OWNER_EMAIL;
      }
      if (!authorized) {
      ...
      const remaining = items.filter((it) => {
        ...
        return it.address !== addressMatch;
      });
      await redis.del(key);
      if (remaining.length) {
  ...
  try {
    ...
    if (!city || !address || !contact) {
      res.status(400).json({ error: 'city, address and contact are required' });
      return;
    }
